...
export class WKRemoteConfig {
  ...
  momentsOff: number = 0; // 是否隐藏朋友圈入口 1:隐藏 0:显示
  aiInvestUrl: string = ""; // AI智投链接
  insightUrl: string = ""; // 洞察链接
  ...
  requestConfig() {
    return WKApp.apiClient.get("common/appconfig").then((result) => {
      ...
      this.momentsOff = result["moments_off"] || 0;
      this.aiInvestUrl = result["ai_invest_url"] || "";
      this.insightUrl = result["insight_url"] || "";
      // 通知更新菜单
      WKApp.menus.refresh();
    });
  }
...
export default class WKApp extends ProviderListener {
  ...
  baseContext!: WKBaseContext; // 飞鸽基础上下文
